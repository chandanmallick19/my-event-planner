<template>    
    <v-form @submit.prevent ref="form">
      <v-card min-width="800">
        ...
        <v-text-field class="mx-6" clearable label="Event title" v-model="name" :rules="nameRules" required></v-text-field>
        <v-textarea class="mx-6" no-resize rows="2" clearable label="Description" v-model="description"></v-textarea>
        <div class="d-flex mx-6">
          <v-select class="mr-2 mt-2" label="Select Date" :items="datesLocalisedArray" v-model="eventDate" required :rules="dateRules"></v-select>
          <v-text-field class="ml-2 mt-2" clearable label="Total tickets" :rules="ticketRules" v-model="total_tickets" required></v-text-field>
        </div>
        <div class="d-flex mx-6">
          <v-select class="mr-2 mt-2" label="Start time" :items="timesArray" :rules="startTimeRules" v-model="event_datetime_start" required></v-select>
          <v-select class="ml-2 mt-2" label="End time" :items="timesArray" :rules="endTimeRules" v-model="event_datetime_end" required></v-select>
        </div>
        <div class="d-flex mx-6">
          <v-btn class="mr-2" density="default" color="blue" @click="findRooms">Find Suitable Rooms</v-btn>
          <v-select class="ml-2" label="Select Room" :disabled="!foundRooms" density="compact" :items="roomsList" item-disabled="" item-title="name" item-value="id" v-model="selectedRoomId" required :rules="findRoomRules"></v-select>
        </div>
        <div class="d-flex mx-6">
          <v-checkbox density="compact" class="mr-2" direction="horizontal" v-model="useGenericImage" label="Use generic event image"></v-checkbox>
          <v-file-input class="ml-2" :disabled="useGenericImage" clearable show-size density="compact" accept=".jpg,.jpeg,.png, image/png, image/jpeg" @change="fileChange" label="Event Advertising Photo" :rules="image_file_nameRules"></v-file-input>
        </div>
        <v-card-actions>
          <v-btn class="mx-2" type="submit" color="blue" @click="publishEvent" :disabled="publishInProgress">Publish Event</v-btn>
    ...
    <v-snackbar v-model="uploadFailed" multi-line color="red">Failed to upload image, please check selection and try again. Event was <b><u>not</u></b> published.
      <template v-slot:actions><v-btn color="white" variant="text" @click="uploadFailed = false">Close</v-btn></template>
    </v-snackbar>
    <v-snackbar v-model="publishFailed" multi-line color="red">Failed to publish event. Please try again.
      <template v-slot:actions><v-btn color="white" variant="text" @click="publishFailed = false">Close</v-btn></template>
    </v-snackbar>
    <v-snackbar v-model="findRoomsFailed" multi-line color="red">{{ findRoomsError }}
      <template v-slot:actions><v-btn color="white" variant="text" @click="findRoomsFailed = false">Close</v-btn></template>
    </v-snackbar>    
  </template>
  ...
  export default {    
    ...
    methods: {
      ...
      async findRooms() {
        ...
        // List all rooms (this will change in a later step)
        const result = await API.graphql({ query: listRooms });
        this.roomsList = result.data.listRooms.items;
      ...
      async publishEvent() {
        ...
        try {
          ...
          if (this.useGenericImage) {
            ...
            const resultKey = await Storage.put(this.imageFile.name, this.imageFile, { level: "protected", });
            storage_key = resultKey;
          }
        } catch (error) {
        ...
        try {
          ...
          consolidated_date_time_start = new Date(eventdate.getFullYear(), eventdate.getMonth(), eventdate.getDate(), eventTimeStart.getHours(), 0, 0, 0);
          consolidated_date_time_end = new Date(eventdate.getFullYear(), eventdate.getMonth(), eventdate.getDate(), eventTimeEnd.getHours(), 0, 0, 0);              
        } catch (error) {
          ...
          const response = await API.graphql({
            ...
            variables: {
              input: {
                ...
                event_datetime_start: consolidated_date_time_start.toISOString(),
                event_datetime_end: consolidated_date_time_end.toISOString(),
